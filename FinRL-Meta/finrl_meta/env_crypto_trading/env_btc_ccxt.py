...
class BitcoinEnv:  # custom env
    def __init__(self, data_cwd=None, price_ary=None, tech_ary=None,
                 time_frequency=15, start=None, mid1=172197, mid2=216837,
                 end=None, initial_account=1e6, max_stock=1e2,
                 transaction_fee_percent=1e-3, mode='train', gamma=0.99):
        self.stock_dim = 1
        ...
        self.gamma = gamma
        self.mode = mode
        self.load_data(data_cwd, price_ary, tech_ary, time_frequency, start, mid1, mid2, end)
    ...
    def reset(self) -> np.ndarray:
        ...
        normalized_tech = [self.day_tech[0] * 2 ** -1, self.day_tech[1] * 2 ** -15, self.day_tech[2] * 2 ** -15,
                           self.day_tech[3] * 2 ** -6, self.day_tech[4] * 2 ** -6, self.day_tech[5] * 2 ** -15,
    ...
    def step(self, action):
        ...
        normalized_tech = [self.day_tech[0] * 2 ** -1, self.day_tech[1] * 2 ** -15, self.day_tech[2] * 2 ** -15,
                           ...
                           self.day_tech[6] * 2 ** -15]
        state = np.hstack(
            (self.account * 2 ** -18, self.day_price * 2 ** -15, normalized_tech, self.stocks * 2 ** -4,)).astype(
            np.float32)
    ...
    def load_data(self, data_cwd, price_ary, tech_ary, time_frequency, start, mid1, mid2, end):
        if data_cwd != None:
            try:
