...
class DataProcessor():
    ...
    def df_to_array(self, df,  if_vix: bool) -> np.array:
        price_array, tech_array, turbulence_array = self.processor.df_to_array(df, self.tech_indicator_list, if_vix)
    ...
    def run(self, ticker_list: str, technical_indicator_list: List[str], if_vix: bool, cache: bool = False, select_stockstats_talib: int = 0):
        ...
        if if_vix:
            self.add_vix()
        price_array, tech_array, turbulence_array = self.df_to_array(self.dataframe, if_vix)
        tech_nan_positions = np.isnan(tech_array)
        tech_array[tech_nan_positions] = 0
