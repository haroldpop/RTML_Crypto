...
class CryptoEnv(gym.Env):  # custom env
    ...
    def step(self, actions): # -> (np.ndarray, float, bool, None):
        ...
        # print('In env.step() after the multiplication with norm_vector')
        for i in range(self.action_dim):
            norm_vector_i = self.action_norm_vector[i]
            actions[i] = actions[i] * norm_vector_i
            # print('i ,actions, norm_vector_i: ', i ,actions, norm_vector_i)
        for index in np.where(actions < 0)[0]:  # sell_index:
            if price[index] > 0:  # Sell only if current asset is > 0
                ...
                # print('sell_num_shares ', sell_num_shares)
...
class CryptoEnv2(gym.Env):  # custom env
    def __init__(self, config, lookback=1, initial_capital=1e6, 
                 buy_cost_pct=1e-3, sell_cost_pct=1e-3, gamma=0.99):
    ...
    def step(self, actions): # -> (np.ndarray, float, bool, None):
        ...
        for index in np.where(actions < 0)[0]:  # sell_index:
            if price[index] > 0:  # Sell only if current asset is > 0
                sell_num_shares = min(self.stocks[index], -actions[index]) #cannot sell more than what we have in stock
                self.stocks[index] -= sell_num_shares
                self.cash += price[index] * sell_num_shares * (1 - self.sell_cost_pct)
                # print('self.stocks[index], actions[index]', self.stocks[index], actions[index])
                # print('sell_num_shares ', sell_num_shares) 
                info["qty"] = - sell_num_shares
        for index in np.where(actions > 0)[0]:  # buy_index:
            if price[index] > 0:  # Buy only if the price is > 0 (no missing data in this particular date)
                buy_num_shares = min(self.cash // price[index], actions[index]) #have to have enough ressources to buy
                self.stocks[index] += buy_num_shares
                self.cash -= price[index] * buy_num_shares * (1 + self.buy_cost_pct)
                # print('self.cash // price[index], actions[index]', self.cash // price[index], actions[index])
                # print('buy_num_shares ', buy_num_shares)
                info["qty"] = buy_num_shares
...
class CryptoEnv3(gym.Env):  # custom env
    def __init__(self, config, lookback=1, initial_capital=1e6, 
                 buy_cost_pct=1e-3, sell_cost_pct=1e-3, gamma=0.99):
