...
MODELS = {"a2c": a2c, "ddpg": ddpg, "td3": td3, "sac": sac, "ppo": ppo, "appo": appo}
...
class DRLAgent:
    ...
    def get_model(
        ...
        if model_name == "a2c":
            ...
            model_config = model.TD3_DEFAULT_CONFIG()
        else:
        ...
        if model_kwargs is not None:
            model_config['lr'] = model_kwargs["lr"]
            model_config['num_workers'] = model_kwargs["num_workers"]
            model_config['train_batch_size'] = model_kwargs["train_batch_size"]
            model_config["gamma"] = model_kwargs["gamma"]
    ...
    def train_model(self, model, model_name, model_config, total_episodes=100):
        ...
        elif model_name == "appo":
            trainer = model.APPOTrainer(env=self.env, config=model_config)
        elif model_name == "a2c":
    ...
    def DRL_prediction(
        ...
        if model_name == "ppo":
            ...
            trainer = MODELS[model_name].APPOTrainer(env=env, config=model_config)
        elif model_name == "a2c":
