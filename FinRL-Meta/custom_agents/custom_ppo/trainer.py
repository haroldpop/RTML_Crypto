...
import time
import os 
from custom_agents.custom_ppo.running_mean import RunningMeanStd
...
def evaluate_model(agent, env):
    ...
    episode_total_actions = list()
    episode_total_assets.append(env_instance.initial_total_asset)
    episode_total_actions.append(0)
    done = False
    while not done:
        action = agent.choose_dist(state)
        action = action.sample()
        state, reward, done, info = env_instance.step(action)
        episode_total_actions.append(info["qty"])
        total_asset = (
            env_instance.cash
            + (env_instance.price_array[env_instance.time] * env_instance.stocks).sum()
        )
        episode_total_assets.append(float(total_asset))
        episode_return = total_asset / env_instance.initial_total_asset
        episode_returns.append(episode_return)  
    return episode_total_assets, episode_total_actions   

class Train:
    def __init__(self, env, test_env, env_name, n_iterations, agent, epochs, mini_batch_size, epsilon, horizon, cwd):
        self.env = env
    ...
    def step(self):
        ...
        for iteration in range(1, 1 + self.n_iterations):
            ...
            episode_total_assets, episode_total_actions = evaluate_model(self.agent, self.test_env)
            #print(episode_total_assets)
            eval_rewards = np.array(episode_total_assets).mean()
    ...
    def print_logs(self, iteration, actor_loss, critic_loss, eval_rewards):
        ...
        if iteration % 100 == 0:
            ...
            self.agent.save_weights(self.cwd, iteration, self.state_rms)
