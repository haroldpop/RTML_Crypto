from custom_agents.custom_ppo.model import Actor, Critic, ActorLSTM, CriticLSTM
...
class Agent:
    def __init__(self, env_name, n_iter, n_states, n_actions, lr):
        ...
        self.current_policy = ActorLSTM(n_states=self.n_states,
                                    n_actions=self.n_actions).to(self.device)
        self.critic = CriticLSTM(n_states=self.n_states).to(self.device)
    ...
    def save_weights(self, cwd, iteration, state_rms):
        torch.save({"current_policy_state_dict": self.current_policy.state_dict(),
                    ...
                    "state_rms_count": state_rms.count}, cwd + self.env_name + "_weights.pth")
